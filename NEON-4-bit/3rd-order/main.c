 ...
 */
  ...
  uint8x16_t x[4], key[4], *w[11];
  int i;
  for (i = 0; i < 11; i++)
    w[i] = (uint8x16_t *)malloc(sizeof(uint8x16_t)*4);
  ...
  key[2] = vxorshf96();
  key[0] = veorq_u8(key[0], key[2]);
  key[3] = vxorshf96();
  key[0] = veorq_u8(key[0], key[3]);
  
  x[1] = vxorshf96();
  x[0] = veorq_u8(x[0], x[1]);   
  x[2] = vxorshf96();
  x[0] = veorq_u8(x[0], x[2]); 
  x[3] = vxorshf96();
  x[0] = veorq_u8(x[0], x[3]); 
